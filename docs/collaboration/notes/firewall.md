...
### 中文动作要**显式**写一条规则（M2-04）

策略表里那条 `production-release-cn`（`actions: ["上线工作流到*"]`）的意义**不是"改档位"**，
而是"让命中可见"：

| | 档位 | `matched` | 含义 |
| --- | --- | --- | --- |
| 没写这条规则 | `approval` | `false` | 掉进 `default_tier` —— **档位看着完全正确**，其实是策略表漏了它 |
| 写了这条规则 | `approval` | `true` | 真的命中了策略 |

两行的**档位一模一样**。如果只看 `tier`，这两种情况永远分不出来 —— 这就是验证报告里
实测过的那个静默失效。所以加动作时请"给它一条专属规则"，而不是指望"反正兜底档也是它要的档"。
（验证方式见 `docs/evidence/issue-10-chinese-action.txt`：同一动作在两份策略表下的 `matched` 对比。）
